...
export class ProgressionService {
  ...
  async getAll(auth0Id: string) {
    ...
    return Array.from(monthCategoryMap.entries()).map(([month, categoryIds]) => {
  ...
  async getByMonth(month: string, auth0Id: string) {
    ...
    if (parts.length !== 2 || isNaN(parts[0]) || isNaN(parts[1])) {
      throw new BadRequestException('Format de mois invalide — attendu : YYYY-MM');
    }
    ...
    const categoryIds = [...new Set(sessions.map((s) => s.categoryId).filter(Boolean))] as string[];
