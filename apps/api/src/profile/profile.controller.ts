import { Controller, Get, Put, Post, Patch, Body, UseGuards, HttpCode, HttpStatus } from '@nestjs/common';
...
export class ProfileController {
    ...
    @Get('stats')
    @ApiOperation({ summary: 'Statistiques du profil', description: "Récupère les statistiques de jeu de l'utilisateur" })
    ...
    @Get('payment-method')
    @ApiOperation({ summary: 'Moyen de paiement', description: "Récupère le moyen de paiement actif de l'utilisateur" })
    @ApiResponse({ status: 200, description: 'Moyen de paiement retourné (null si aucun)' })
    @ApiResponse({ status: 401, description: 'Non autorisé' })
    ...
    @Get('plans')
    @ApiOperation({ summary: 'Plans disponibles', description: "Récupère tous les plans d'abonnement avec le plan actuel de l'utilisateur" })
    ...
    @ApiOperation({ summary: 'Choisir un plan', description: "Change le plan d'abonnement de l'utilisateur" })
    @ApiBody({
        schema: {
            type: 'object',
            required: ['slug'],
    ...
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: "Résilier l'abonnement", description: "Résilie l'abonnement à la fin de la période en cours" })
