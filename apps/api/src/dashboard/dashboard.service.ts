...
const BADGE_META: Record<
  string,
  { gradient: string; levelLabel: string; barColor: string }
> = {
  FIRST_SESSION: {
  ...
  TOTAL_QUESTIONS: {
    gradient: 'linear-gradient(135deg,#2dd4bf,#22c55e)',
    levelLabel: 'Bronze',
    barColor: '#2dd4bf',
  },
  PERFECT_SESSION: {
...
export class DashboardService {
  ...
  async getStats(auth0Id: string) {
    ...
    return [
      ...
      { id: 'xp', label: 'XP total', target: user.profile?.xpTotal ?? 0 },
      {
        ...
        target: user.profile?.streakDays ?? 0,
        suffix: ' j',
      },
    ];
  }
  ...
  async getAchievements(auth0Id: string) {
    ...
    const totalQuestions = user.quizSessions.reduce(
      (s, q) => s + q.totalQuestions,
      0,
    );
    ...
    return allBadges.map((badge) => {
      ...
      switch (badge.conditionType) {
        case 'FIRST_SESSION':
          progress = Math.min(completedSessions, badge.conditionValue);
          break;
        case 'STREAK_DAYS':
          progress = Math.min(currentStreak, badge.conditionValue);
          break;
        case 'TOTAL_XP':
          progress = Math.min(totalXp, badge.conditionValue);
          break;
        case 'TOTAL_QUESTIONS':
          progress = Math.min(totalQuestions, badge.conditionValue);
          break;
        case 'PERFECT_SESSION':
          progress = Math.min(perfectSessions, badge.conditionValue);
          break;
        default:
          progress = unlocked ? badge.conditionValue : 0;
      }
