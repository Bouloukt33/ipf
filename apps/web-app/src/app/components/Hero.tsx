...
export default function Hero() {
    ...
    return (
        <section className="min-h-screen pt-24 pb-12 px-6 md:px-12 relative overflow-hidden flex items-center bg-gradient-to-br from-[#0F1419] via-[#1A2332] to-[#0F1419]">
                    ...
                    <div className={`relative transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-x-0 scale-100' : 'opacity-0 translate-x-8 scale-95'}`}>
                        <div className="relative w-full aspect-square max-w-[600px] mx-auto">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                {/* Rotating ring */}
                                <div className="absolute w-[90%] h-[90%] rounded-full border-2 border-dashed border-primary/20 animate-spin-slow" />
                                
                                {/* Glow ring */}
                                <div className={`absolute w-[85%] h-[85%] rounded-full bg-gradient-to-br ${mascots[currentMascot].gradient} blur-3xl animate-pulse-slow`} />
                            </div>
                                ...
                                {mascots.map((mascot, index) => (
                                    ...
                                    >
                                        <Image
                                            ...
                                            alt="Mascotte"
                                            fill
                                            className="object-contain drop-shadow-[0_20px_80px_rgba(0,0,0,0.5)] animate-float-gentle"
                                            priority={index === 0}
                                        />
                                    </div>
                                ))}
